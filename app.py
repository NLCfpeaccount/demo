...
#  Fixed config (Feasibility Lab verdicts). Cloud-hardened. No knobs.
...
# ---------------- 1 · FIXED CONFIG (locked lab verdict) ----------------
...
def clean_text(raw: str) -> str:
    """Strip page footers, trailing spaces, blank-line runs. Regex only, no AI."""
...
SCHEMA_PROMPT = (
    ...
    "3. Types: name=str; top_skills=list[str] max 6; years_experience=number (total years, 0 if unknown); "
    "education=str; key_projects=list[str] max 3, one line each; summary=str one sentence.\n"
...
class Profile(BaseModel):
    """The intake form. Pydantic = the CHECKER's rulebook."""
...
def parse_json_defensive(raw: str):
    ...
    truncated outputs by closing open brackets. Raises ValidationError if all fail."""
    ...
    for cand in (s, s + "}", s + "]}", s + "\"}", s + "]}", s + "\"}"):
        try: return Profile.model_validate_json(cand)
...
def get_llm():
    ...
    variants = []
    if "cache_type_k" in params: variants.append({"cache_type_k": "q8_0"})  # K-cache quant, no flash_attn needed
    ...
    for att in (1, 2):
        ...
        try: return parse_json_defensive(raw), att, secs, None
            ...
            msgs += [{"role": "assistant", "content": raw},
                     {"role": "user", "content":
                      f"Errors: {e.errors()[:2]}. Fix ONLY these. Keep every other value. "
                      "Reply with the complete JSON object only, starting with { and ending with }."}]
...
with st.sidebar:
    ...
    st.divider()
    st.caption("Fixed build: Qwen2.5-0.5B · q4_k_m · ctx4096 · thr2 · q8 K-cache. "
